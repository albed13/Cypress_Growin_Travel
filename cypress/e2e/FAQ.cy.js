...
it('should login with valid credentials', () => {
    ...
    cy.get('#faq_question').type('How much is the tour package to Bali?')
    cy.get('#faq_answer').type('The tour package to Bali starts from IDR 5,000,000 per person, including accommodation, transportation, and a 4 days 3 nights tour.')
    cy.get('#submitButton').click()
    cy.get('.swal2-confirm').click()

    //pencarian
    cy.get('#searchName').type('How much is the tour package to Bali?')
    cy.get('#searchName').clear()
    cy.get('#faqCategoryFilterDropdown').click()
    ...
    //edit
    cy.get(':nth-child(3) > .text-center > .list-unstyled > :nth-child(2) > a > .feather').click()
    cy.get('#categoryDropdown').click()
